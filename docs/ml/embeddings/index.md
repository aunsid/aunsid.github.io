...
## Token Embeddings
After the text is broken down into raw textual units(tokens), and the corresponding IDs have been assigned to these tokens; to convert these token IDs into something meaningful that can be understood by the model, an embedding table is used by the model. One can think of it as a lookup table, a matrix of vocab_size x embedding_dim. For each token ID, the table is indexed and the token's corresponding embedding is retrieved.
...
~~1. Sets of positions needs to be decided ahead of time~~ 
2. May hinder generalization of tokens in different positions. 
