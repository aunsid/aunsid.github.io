...
Large language models(llms) cannot work with text in its raw form (strings). The text needs to be converted into an integer representation first. The process of breaking input sequence down into smaller units so that it can be used by llms is called tokenization. There are various ways this can be done - characters, subwords, and words. Each method has its strengths and drawbacks.

### Characters
The easiest way to assign ids to text, is to breakdown the text into characters. And each character gets a unique token. An example of this would be for the input sequence - This is an example text.
...
* s - 3
* ' ' - 4
* i - 2 # assigning i and s as the same token ids as they have been identified before 
* s - 3
* ' ' - 4 # seen before
* a - 5
* n - 6
...
### Embeddings
TokenIDs are straight forward they are unique ids that each part of the sequence receives, similar to vectorization. They do not capture any semantic relationships between tokens.
On the other hand, embeddings are advanced vector representations that provide semantic meaning to the tokens based on how commonly they are used together with, or in similar contexts to, the other tokens. These embeddings are learnt by the llm during training. 


Workflow:
Text -> tokenids -> vector embeddings -> output logits over vocabs ->  tokenids -> Text

The way text is processed in a LLM workflow is first text is converted into tokenids using a tokenizer. These tokenids are then passed to the model for processing. The model computes overall embeddings based on individual embeddings on the learned tokens. During output generation, the model provides the vector value/embedding for the next token over the entire vocabulary. We then select the most probable token and convert it back into text.
